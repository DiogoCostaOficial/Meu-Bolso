// Script para conceder permissão de administrador ao usuário admin@admin.com
import fs from 'fs';
...
console.log('🔐 Iniciando concessão de permissão de administrador...');

// Função para fazer login como admin com senha específica
function loginComoAdminComSenha(senha) {
            ...
            res.on('end', () => {
                try {
                    ...
                    if (result.success || result.token) {
                        console.log('✅ Login como admin@admin.com realizado com sucesso!');
                        resolve(result.token);
                    } else {
                        reject(new Error('Erro no login: ' + (result.message || 'Erro desconhecido')));
                    }
                } catch (e) {
                    reject(new Error('Erro ao parsear resposta do login: ' + e.message));
                }
            });
        });
...
// Função para fazer login como admin e obter token
...
// Função para verificar dados atuais do usuário
...
async function main() {
    try {
        ...
        console.log('\n🔐 Fazendo login como admin@admin.com...');
        const token = await loginComoAdmin().catch(() => {
            console.log('\n⚠️  Tentando com senha padrão admin123...');
            return loginComoAdminComSenha('admin123');
        });
